      ...
      <div className="container-custom">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 xl:gap-12 2xl:gap-12 items-center">
          <div ref={leftRef} className="min-w-0">
            <span className="eyebrow-mdk mb-2 block">Why Choose Tradeware</span>
            <h2 id="why-choose-us-heading" className="heading-section-mdk text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-slate-900 mb-3 md:mb-4 tracking-tight leading-tight">
              Your Trusted Partner for <span className="text-red-600">Japanese Vehicle Imports</span>
            </h2>
            <p className="text-slate-600 mb-4 md:mb-6 max-w-lg leading-relaxed text-xs sm:text-sm md:text-base break-words">
            ...
            <div ref={highlightsRef} className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4 md:mb-6" role="list">
              {highlights.map((h) => {
                ...
                return (
                  <div
                    key={h.title}
                    className="group flex gap-2.5 sm:gap-3 min-w-0 p-2.5 rounded-xl border border-slate-200 hover:border-red-500/30 transition-all duration-300 card-animate-hover bg-white"
                    role="listitem"
                  >
                    <div className="shrink-0 w-8 h-8 sm:w-9 sm:h-9 rounded-lg border border-slate-200 bg-red-500/10 flex items-center justify-center icon-glow-hover icon-pulse-hover animate-float-subtle" aria-hidden="true">
                      <Icon className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-red-600" />
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-sm font-semibold text-slate-900 mb-0.5 break-words">{h.title}</h4>
                      <p className="text-[11px] sm:text-xs text-slate-600 break-words">{h.desc}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div ref={rightRef} className="relative min-w-0">
            ...
            <div className="absolute -bottom-3 -right-3 md:-right-4 rounded-xl border border-slate-200 bg-white p-3.5 shadow-xl max-w-[160px]">
              <div className="text-xl font-bold text-red-600">99.8%</div>
              <div className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Customer Satisfaction</div>
            </div>
          </div>
        </div>
      </div>
